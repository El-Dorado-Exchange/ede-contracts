    ...
    DataStore
{
    ...
    bytes32 public constant ACCUM_FEE_REBATED_CLAIMED =
        keccak256("ACCUM_FEE_REBATED_CLAIMED");
    bytes32 public constant ACCUM_FEE_DISCOUNTED_CLAIMED =
        keccak256("ACCUM_FEE_DISCOUNTED_CLAIMED");
    ...
    constructor(address _NFTUtils) {
        ...
        ESBTStr memory _ESBTStr = ESBTStr(
            address(this),
            "ESBT OFFICIAL",
            defRC,
            cur_time,
        ...
        scorePara[8] = 10; //invite create Account
    ...
    function setUintValueByString(
        string memory _strIdx,
        uint256 _value
    ) public onlyOwner {
    ...
    function setAcitvity(address _act, bool _add) public onlyOwner {
        if (_add) safeGrantAddressSet(ONLINE_ACTIVITIE, _act);
        else safeRevokeAddressSet(ONLINE_ACTIVITIE, _act);
    }
    ...
    function setScoreToRank(uint256[] memory _minValue) external onlyOwner {
        require(
            _minValue.length > 3 && _minValue[0] == 0,
            "invalid score-rank setting"
        );
    ...
    function setDisReb(
        ...
        require(
            scoreToRank.length + 1 == _dis.length && _dis.length == _reb.length,
            "invalid dis-reb setting"
        );
        rankToReb = _reb;
        rankToDis = _dis;
    }

    function setScorePlan(
        uint256 _decPerDay,
        uint256 _decInterval,
        uint256 _rankUpdInterval
    ) external onlyOwner {
        require(
            _decPerDay < SCORE_DECREASE_PRECISION,
    ...
    function _mint(
        ...
        require(
            _referalAccount != address(0) && balanceOf(_referalAccount) > 0,
        ...
        uint256 new_rank = rankByScore(getAddUint(_newAccount, ACCUM_SCORE));
        emit RankUpdate(
            _newAccount,
            _tokens[addressToTokenID[_newAccount]].rank,
            new_rank
        );
        ...
        updateScore(
            _referalAccount,
            _newAccount,
            scorePara[8] * SCORE_PRECISION,
            0
        );
        if (getAddressSetCount(ONLINE_ACTIVITIE) > 0) {
            address[] memory actList = getAddressSetRoles(
                ONLINE_ACTIVITIE,
                0,
                getAddressSetCount(ONLINE_ACTIVITIE)
            );
    ...
    function rankByScore(uint256 _score) public view returns (uint256) {
    ...
    function rank(address _account) public view override returns (uint256) {
    ...
    function updateFee(
        ...
        uint256 _discountedFee = _origFee.mul(dis_per).div(
        ...
        address _parent = getAddMpAddressSetRoles(
            _account,
            REFERRAL_PARRENT,
            0,
            1
    ...
    function updateClaimVal(
        address _account
    ) external override onlyScoreUpdater {
        setAddUint(
            _account,
            ACCUM_FEE_REBATED_CLAIMED,
            getAddUint(_account, ACCUM_FEE_REBATED)
        );
        setAddUint(
            _account,
            ACCUM_FEE_DISCOUNTED_CLAIMED,
            getAddUint(_account, ACCUM_FEE_DISCOUNTED_CLAIMED)
        );
    }
    ...
    function updateScoreForAccount(
        ...
        updateScore(
            _account,
            _account,
            _amount.div(1000).mul(scorePara[_reasonCode]).div(
        ...
        if (_par.length == 1)
            updateScore(
                _par[0],
                _account,
                _amount.div(1000).mul(scorePara[1000 + _reasonCode]).div(
                    USD_TO_SCORE_PRECISION
                ),
                1000 + _reasonCode
            );
    ...
    function updateTradingScoreForAccount(
        ...
        updateScore(
            _account,
            _account,
            _amount.div(1000).mul(scorePara[1 + _refCode]).div(
                USD_TO_SCORE_PRECISION
            ),
            1 + _refCode
        );
        if (_par.length == 1)
            updateScore(
                _par[0],
                _account,
                _amount.div(1000).mul(scorePara[2 + _refCode]).div(
                    USD_TO_SCORE_PRECISION
                ),
                11 + _refCode
            );
    ...
    function updateSwapScoreForAccount(
        ...
        updateScore(
            _account,
            _account,
            _amount.div(1000).mul(scorePara[3]).div(USD_TO_SCORE_PRECISION),
            2
        );
        if (_par.length == 1)
            updateScore(
                _par[0],
                _account,
                _amount.div(1000).mul(scorePara[4]).div(USD_TO_SCORE_PRECISION),
                12
            );
    }
    ...
    function updateAddLiqScoreForAccount(
        ...
        updateScore(
            _account,
            _account,
            _amount.div(1000).mul(scorePara[5 + _refCode]).div(
                USD_TO_SCORE_PRECISION
            ),
            3 + _refCode
        );
        if (_par.length == 1)
            updateScore(
                _par[0],
                _account,
                _amount.div(1000).mul(scorePara[6 + _refCode]).div(
                    USD_TO_SCORE_PRECISION
                ),
                13 + _refCode
            );
    ...
    function updateScore(
        ...
        if (timeError > getUint(INTERVAL_SCORE_UPDATE)) {
            ...
            uint256 decreaseAmount = cur_score
                .mul(timeError.div(24 * 3600).mul(scoreDecreasePercentPerDay))
        ...
        if (
            timeError > getUint(INTERVAL_RANK_UPDATE) &&
            balanceOf(_account) == 1
        ) {
            setAddUint(_account, TIME_RANK_UPD, block.timestamp);
            uint256 new_rank = rankByScore(getAddUint(_account, ACCUM_SCORE));
            emit RankUpdate(
                _account,
                _tokens[addressToTokenID[_account]].rank,
                new_rank
            );
    ...
    function getReferralForAccount(
        address _account
    ) public view override returns (address[] memory, address[] memory) {
    ...
    function userSizeSum(
        address _account
    ) public view override returns (uint256) {
        return
            getAddUint(_account, ACCUM_POSITIONSIZE)
                .add(getAddUint(_account, ACCUM_SWAP))
                .add(getAddUint(_account, ACCUM_ADDLIQUIDITY));
    }
    ...
    function userClaimable(
        ...
        return (
            getAddUint(_account, ACCUM_FEE_REBATED).sub(
                getAddUint(_account, ACCUM_FEE_REBATED_CLAIMED)
            ),
            getAddUint(_account, ACCUM_FEE_DISCOUNTED).sub(
                getAddUint(_account, ACCUM_FEE_DISCOUNTED_CLAIMED)
            )
        );
    }
    ...
    function getScore(
        ...
        uint256 decreaseAmount = cur_score
            .mul(pastDays.mul(scoreDecreasePercentPerDay))
            .div(SCORE_DECREASE_PRECISION);
