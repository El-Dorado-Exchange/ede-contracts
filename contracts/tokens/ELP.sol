...
contract ELP is IERC20, IMintable, ReentrancyGuard, IELP, Ownable {
    ...
    event buyESUD(address account, address token, uint256 amount, uint256 fee);

    event sellESUD(address account, address token, uint256 amount, uint256 fee);
    ...
    function transferFrom(
        ...
        uint256 nextAllowance = allowances[_sender][msg.sender].sub(
            _amount,
            "ELP: transfer amount exceeds allowance"
        );
    ...
    function _burn(address _account, uint256 _amount) internal {
        ...
        balances[_account] = balances[_account].sub(
            _amount,
            "ELP: burn amount exceeds balance"
        );
    ...
    function _transfer(
        ...
        balances[_sender] = balances[_sender].sub(
            _amount,
            "ELP: transfer amount exceeds balance"
        );
    ...
    function USDbyFee() external view override returns (uint256) {
    ...
    function _updateRewardsLight(address _account) private {
        ...
        uint256 accountReward = accountAmount
            .mul(
        ...
        returns (uint256)
    {
        ...
        uint256 amountToEDEPool = _mintEUSDAmount.mul(feeToPoolRatio).div(
            feeToPoolPrec
        );
        ...
        if (supply > 0 && thisRewardAmount > 0) {
            _cumulativeRewardPerToken = _cumulativeRewardPerToken.add(
                thisRewardAmount.mul(REWARD_PRECISION).div(supply)
            );
        }
        uint256 accountAmount = balances[_account].add(stakedAmount[_account]);
        uint256 accountReward = accountAmount
        ...
        returns (uint256)
    {
        ...
        if (tokenAmount > 0) {
            require(
                EUSDELPRewardClaimed.add(tokenAmount) <= EUSDELPReward,
                "EUSD Reward out of range"
            );
    ...
    function _pendingRewards() private returns (uint256) {
        ...
        uint256 _mintEUSDAmount = IVault(vault).claimFeeReserves().div(
            PRICE_TO_EUSD
        );
        ...
        returns (uint256)
    {
        ...
        for (uint64 i = 0; i < _stasticDays; i++) {
            _feeTotal = _feeTotal.add(
                IVault(vault).feeReservesRecord(currentIndex.sub(i))
            );
        }
    ...
    function withdrawToEDEPool() external override returns (uint256) {
        uint256 extAmount = 0;
        if (
            edeStakingPool != (address(0)) &&
            EUSDEDEReward > EUSDEDERewardClaimed
        ) {
