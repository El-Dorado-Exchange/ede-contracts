...
contract gEdeYieldDistributor is Owned, ReentrancyGuard {
    ...
    function fractionParticipating() external view returns (uint256) {
        return
            totalgEDEParticipating.mul(PRICE_PRECISION).div(
                totalgEDESupplyStored
            );
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        (
            uint256 eligible_current_gEDE,
            uint256 ending_timestamp
        ) = eligibleCurrentgEDE(account);
        ...
        uint256 gEDE_balance_to_use;
        {
            ...
            if (eligible_current_gEDE > old_gEDE_balance) {
                ...
                gEDE_balance_to_use = (
                    (eligible_current_gEDE).add(old_gEDE_balance)
                ).div(2);
            }
        }
        ...
        returns (uint256 yield0)
    {
        ...
        if (yield0 > 0) {
            yields[from] = 0;
            yieldAmount = yieldAmount.add(yield0);
            TransferHelper.safeTransfer(emitted_token_address, from, yield0);
    ...
    function estimateWeekYield(address account) public view returns (uint256) {
        ...
        if (
            block.timestamp > lastRewardClaimTime[account] && currentProfit > 0
        ) {
            profitDuringWeek = block
                .timestamp
