...
contract TokenVesting_alphaEDE is Ownable {
    ...
    uint256[] public rankVestingDurationList = [
        365 days,
        365 days,
        180 days,
        150 days,
    ...
    constructor(address _principalToken, address _payOutToken, address _esbt) {
        require(
            IERC20(_payOutToken).decimals() ==
                IERC20(_principalToken).decimals(),
            "decimals mismatch"
        );
    ...
    function setRankVestingDurationList(
        uint256[] memory _rankVestingDurationList
    ) external onlyPolicy {
        rankVestingDurationList = _rankVestingDurationList;
    }

    function payOutOfTokenAmount(
        uint256 _amount
    ...
    function deposit(uint _amount) external returns (uint) {
        ...
        require(
            totalPayoutGiven <= payoutToken.balanceOf(address(this)),
